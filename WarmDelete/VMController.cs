...
namespace VmcController.Services
{
    ...
    public class DetectOpenFiles : ServicedComponent
    {
        ...
        public class HandleRecord
        {
            ...
            private IntPtr duplicatedHandle = IntPtr.Zero;
            ...
            private void DuplicateHandle()
            {
                ...
                if (!DuplicateHandle(Process.GetProcessById(ProcessId).Handle, Value,
                    Process.GetCurrentProcess().Handle, out duplicatedHandle,
                    (uint)DuplicateOptions.DUPLICATE_SAME_ACCESS, false, (uint)DuplicateOptions.DUPLICATE_CLOSE_SOURCE))
                {
            ...
            public void Close()
            {
                ...
                Log.Info($"Closed Handle {Value}");
            }

            public BY_HANDLE_FILE_INFORMATION GetHandleFileInfo()
            {
                BY_HANDLE_FILE_INFORMATION result;
                if(!GetFileInformationByHandle(Value, out result))
                {
                    throw new Win32Exception(Marshal.GetLastWin32Error());
                }
                return result;
            ...
            public bool IsOpen
            {
                get
                {
                    DuplicateHandle();
                    BY_HANDLE_FILE_INFORMATION result;
                    return GetFileInformationByHandle(Value, out result);
                }
            }
        }
        ...
        /// <returns></returns>
        public static IEnumerable<HandleRecord> GetOpenFilesEnumerator(int processId)
        {
            return new OpenFiles(processId);
        }

        private sealed class OpenFiles : IEnumerable<HandleRecord>
        {
            ...
            public IEnumerator<HandleRecord> GetEnumerator()
            {
                NT_STATUS ret;
                int length = 0x10000;
                                ...
                                if (handleEntry.OwnerPid == processId)
                                {
                                    ...
                                    if (GetHandleType(handle, handleEntry.OwnerPid, out handleType) && handleType == SystemHandleType.OB_TYPE_FILE)
                                    {
                                        ...
                                        if (GetFileNameFromHandle(handle, handleEntry.OwnerPid, out devicePath))
                                        {
                                            ...
                                            {
                                                yield return new HandleRecord
                                                {
                                                    ProcessId = processId,
                                                    Value = (IntPtr) handleEntry.HandleValue,
                                                    FilePath = dosPath
                                                };
                                            }
                                        }
